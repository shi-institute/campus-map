...
export abstract class SvelteYAbstractType<T, E> extends Y.AbstractType<E> {
  ...
  protected toReactiveSharedType<V>(
    value: V,
    origin?: string,
    additionalTransformers?: SvelteYGetterTransformer[]
  ): V {
    for (const [proto, transformer] of additionalTransformers || []) {
      if (value instanceof proto) {
        return transformer(value, origin);
      }
    }
    if (value instanceof Y.Array && SvelteYAbstractType.__registeredTypes.has('SvelteYArray')) {
      ...
      return new SvelteYArray(() => value, origin) as V;
    }
    if (value instanceof Y.Map && SvelteYAbstractType.__registeredTypes.has('SvelteYMap')) {
      ...
      return new SvelteYMap(() => value, origin) as V;
    }
    return value;
  }
}

export type SvelteYGetterTransformer<P = any, R = any> = [
  proto: P,
  transformer: (v: any, origin?: string) => R,
];
